...
bool GLOBALVARS::createAllOutputFiles(QDate &today)
{
    ...
    root = QString("UpdateLegacyDeceasedrunTimeError");
    ...
    root = QString("UpdateLegacyDeceasedurlError");
    ...
    root = QString("UpdateLegacyDeceasedconnectionError");
    ...
    root = QString("UpdateLegacyDeceasedsqlError");
    ...
    root = QString("UpdateLegacyDeceasedactionRequired");
    ...
    root = QString("UpdateLegacyDeceasedrunSummary");
    ...
    root = QString("UpdateLegacyDeceasedauditListing");
...
void GLOBALVARS::logMsg(msgType messageType, QString msg, int msgInfo)
{
    ...
    case RunSummary:
        if(FHrunSummary->open(QIODevice::WriteOnly | QIODevice::Append | QIODevice::Text))
        {
            ...
            *outputStream << "UpdateLegacyDeceased stat: " << msg << Qt::endl;
            FHrunSummary->close();
        }
        break;
...
bool GLOBALVARS::determineIfBatchRun()
{
    ...
    if (dateForName.time() > QTime(21, 30))
        dateForName = dateForName.addDays(1);

    QString targetFileName(QString("C:\\Obits\\Batch Read\\Nightly LEGACY Obit Run Log for "));
