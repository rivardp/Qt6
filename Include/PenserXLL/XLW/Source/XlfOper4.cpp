...
// PMR - Added IsValidIDV 2008-07-19
...
// Added by PMR - Performs basic validity checks ensuring proper input form to Penser - More
// validity checks completed by Penser

bool xlw::XlfOper4::IsNumVector() const {
	...
	short int r = this->rows();
	short int c = this->columns();
	if((r > 1) && (c > 1))
		return false;
	...
	for (i=0; i<r; i++)
		for (j=0; j<c ; j++)
		{
		    xloper &element = lpxloper_->val.array.lparray[i*c + j];
			OK = OK && (element.xltype == xltypeNum);
		};
