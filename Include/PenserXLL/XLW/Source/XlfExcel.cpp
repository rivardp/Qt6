...
/*
 Copyright (C) 1998, 1999, 2001, 2002, 2003, 2004 Jerome Lecomte
...
#include <XlfOperImpl4.h>
#include <XlfOperImpl12.h>
...
void xlw::XlfExcel::MsgBox(const char *errmsg, const char *title) {
    ...
    if (!title) {
		...
		MessageBox(NULL, LPCWSTR(completeMessage), _T("XLL Error"), MB_OK | MB_ICONINFORMATION);
        // Free the buffer.
        LocalFree(lpMsgBuf);
    } else {
		MessageBox(NULL, LPCWSTR(errmsg), LPCWSTR(title), MB_OK | MB_ICONINFORMATION);
    }
    return;
}
...
void xlw::XlfExcel::InitLibrary() {
    HINSTANCE handle = LoadLibrary(_T("XLCALL32.DLL"));
    if (handle == 0)
...
bool CALLBACK EnumProc(HWND hwnd, LPEnumStruct pEnum) {
    ...
    GetClassName(hwnd, (LPWSTR)rgsz, CLASS_NAME_BUFFER);
    if (2 == CompareString(MAKELCID(MAKELANGID(LANG_ENGLISH,
        SUBLANG_ENGLISH_US),SORT_DEFAULT), NORM_IGNORECASE,
        (LPWSTR)rgsz,  (strlen(rgsz)>lstrlen(LPCWSTR("bosa_sdm_XL")))
        ? lstrlen(LPCWSTR("bosa_sdm_XL")):-1, LPCWSTR("bosa_sdm_XL"), -1)) {
