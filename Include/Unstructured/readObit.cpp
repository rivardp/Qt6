...
//                uc = getUntil("<div class=");
    ...
    case DomainFuneraire:
        if (consecutiveMovesTo(1000, "class=\"description\"", "<p class=\"js-", ">"))
            uc = getUntil("<div class=\"sympathy-messages\">");
        break;
    ...
    case Citrus:
    {
        if (consecutiveMovesTo(10000, "</div></div></div>", "</div></div></div>", "</div></div></div>"))
        {
            moveBackwardTo("jet-listing-dynamic-field__content");
            moveTo(">");
    ...
    case Carve:
    {
        if (consecutiveMovesTo(150, "id=\"death_notice\"", ">Death Notice<", ">"))
            uc = getUntil("</div>");
    }
        break;
    ...
    case Globalia:
    {
        if (moveTo("class=\"single-content\""))
            uc = getUntil("</div>");
    }
        break;
    ...
    case PubliWeb:
    {
        ...
        {
        case 0:
            if (moveTo("<div class=\"f_main_desc\">"))
                uc = getUntil("</div>");
            break;
    ...
    case Canadian:
    {
        consecutiveMovesTo(100, "class=\"fastsocialshare_clearer\"", ">");
        uc = getUntilEarliestOf("DurhamObituaries.com</a>", "TorontoObituaries.com</a>");
    }
        break;
    ...
    {
        simplify();
        target = OQString(globals->globalDr->getTitle()).convertFromID();
        if (moveTo(target))
        {
            moveTo("</b>");
            uc = getUntil("</td>");
    ...
    case Alcock:
        if (consecutiveMovesTo(4000, "data-testid=\"richTextElement\"", "</p>"))
        {
            moveBackwardTo("<span", 4000);
            moveTo(">");
            uc = getUntil("</div>");
        }
        break;
    ...
    case FirstMemorial:
        if (consecutiveMovesTo(150, "class=\"Obituaries-details\"", "</h", ">"))
            uc = getUntil("<!-- You can start editing here. -->");
        break;
    ...
    case Parent:
        if (consecutiveMovesTo(100, "class=\"wpfh-clear", "</div>"))
            uc = getUntil("<div class=\"screen-reader-response\">");
        break;
    ...
    case LifeTransitions:
        if (consecutiveMovesTo(100, "class=\"entry-content content\"", ">"))
            uc = getUntil("div class=\"et_social_inline");
        break;
    ...
    case Hamel:
    {
        if (consecutiveMovesTo(100, "id=\"the-obit-info\"", "</p>"))
            uc = getUntil("</div>");
    }
        break;
...
void readObit::readInObitTitle()
{
    ...
    case Carve:
    {
        if (consecutiveMovesTo(100, "id=\"content\"", "<h"))
            titleStream = readNextBetween(BRACKETS);
    }
        break;
    ...
    case Rajotte:
        ...
        if (moveTo(targetText))
        {
            moveTo(QLatin1String("Avis de décès - "));
            titleStream = getUntil(" - ");
        }
        break;
...
QStringList readObit::getLocationWords(PROVIDER provider, unsigned int providerKey)
{
    ...
    case 6:
    {
        QList<QLatin1String> tempLocations;
        switch(providerKey)
        {
        case 45:
            tempLocations.append(QLatin1String("dekebaowek"));
            tempLocations.append(QLatin1String("dekipawa"));
            tempLocations.append(QLatin1String("demême"));
            tempLocations.append(QLatin1String("detémiscaming"));
            tempLocations.append(QLatin1String("dethorne"));
            tempLocations.append(QLatin1String("dewinneway"));
            break;
...
void readObit::readMessages()
{
    ...
    case LifeTransitions:
        commentDate = globals->today;
        while (consecutiveMovesTo(250, "class=\"x-comment-time\"", "datetime=\""))
        {
            tempUC = getUntil("T");
            tempDate = tempUC.readDateField(doYMD);
            if (tempDate.isValid() && (tempDate < commentDate))
    ...
    if (globals->globalDr->getAgeAtDeath() == 0)
    {
        ...
        while (!ucCleaned.isEOS() && (sentenceCount < 5) && keepGoing)
        {
            ...
            if (sentence.getLength() > 0)
            {
                ...
                QStringList frenchWords = QString(QLatin1String("était|ans|an")).split("|");
...
void readObit::readStructuredContent()
{
    ...
    case DomainFuneraire:
        if (consecutiveMovesTo(100, "<title>", "Avis", " - "))
        {
            QString tempString = getUntilEarliestOf(" |", "<").getString();
            QRegularExpression target;
            target.setPattern("\\*\\*\\w+\\*\\*");
            tempString.replace(target, "");
            target.setPattern(" \\d{4}-\\d{4}");
            tempString.replace(target, "");
            tempUC = tempString;
            tempUC.processStructuredNames(nameStatsList, fixHyphens);
        }
   ...
   case SiteWyze:
    ...
    case RKD:
    {
        ...
        {
            beg();
            if (moveTo("elementor-heading-title elementor-size-default"))
            {
                tempUC = readNextBetween(BRACKETS);
                tempUC.processStructuredNames(nameStatsList, fixHyphens);
    ...
    case PubliWeb:
    {
        ...
        {
        case 0:
            if (moveTo("class=\"f_main_date\""))
            {
                tempUC = readNextBetween(BRACKETS);
                tempUC.processDateField(dfDOD);

                if (moveTo("class=\"f_main_death_name\""))
                {
                    tempUC = readNextBetween(BRACKETS);
                    tempUC.processStructuredNames(nameStatsList, fixHyphens);
                }
    ...
    case Alcock:
        if (consecutiveMovesTo(4000, "data-testid=\"richTextElement\"", "</p>"))
        {
            ...
            OQStream tempStream(tempUC);
            tempTempUC.clear();
            int wordCount = 0;
            bool keepGoing = true;
            while (!tempStream.isEOS() && (wordCount <= 6) && keepGoing)
            {
                word = tempStream.getWord(true);
                wordCount++;
                if ((wordCount > 2) && word.isRecognized())
                    keepGoing = false;
                if (keepGoing)
                {
                    tempTempUC += word;
            ...
            tempTempUC.processStructuredNames(nameStatsList, fixHyphens);
        }
        break;
    ...
    case FirstMemorial:
        if (consecutiveMovesTo(100, "class=\"Obituaries-details\"", "<h"))
        {
            tempUC = readNextBetween(BRACKETS);
            tempUC.processStructuredNames(nameStatsList, fixHyphens);
        }
    ...
    case Jacques:
        if (consecutiveMovesTo(100, "class=\"item-title\"", ">"))
        {
            ...
            if (consecutiveMovesTo(10, "<p>", "le "))
            {
                ...
                if (moveTo(QLatin1String("à l’âge de "), 15))
                {
                    tempUC = getUntil(" ");
                    tempUC.processAgeAtDeath();
                }
    ...
    case Parent:
        if (consecutiveMovesTo(600, "class=\"wpfh_main_obit_info", "<a", ">"))
        {
            backward(1);
            tempUC = readNextBetween(BRACKETS);
            tempUC.processStructuredNames(nameStatsList, fixHyphens);
    ...
    case Hamel:
        if (consecutiveMovesTo(25, "id=\"the-obit-info\"", "<h"))
        {
            tempUC = readNextBetween(BRACKETS);
            tempUC.processStructuredNames(nameStatsList, fixHyphens);
